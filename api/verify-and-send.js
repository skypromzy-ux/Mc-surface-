...
export default async function handler(req, res) {
  ...
  const rows = order
    .map(
      (i) =>
        `<tr><td style="padding:8px 12px;border-bottom:1px solid #EAE2D2;">${i.name}</td><td style="padding:8px 12px;border-bottom:1px solid #EAE2D2;text-align:center;">${i.qty}</td><td style="padding:8px 12px;border-bottom:1px solid #EAE2D2;text-align:right;">\u20a6${(i.price * i.qty).toLocaleString()}</td></tr>`
    )
    .join("");
